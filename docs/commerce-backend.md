...
- `GET /api/admin/media-integrity`：管理员只读核对 D1 媒体记录与私有 R2 对象，报告缺失对象、孤立对象、未关联 active 媒体和异常作品引用；不会自动删除或修复。
...
## 阶段 5E：Preview 运维一致性（已完成首轮验收）

- 后台新增“检查图片一致性”，通过受保护的只读 API 分页列出 R2 对象并与 D1 媒体元数据和作品引用比对；异常只报告，不自动清理。
- 2026-08-26 实际检查结果为 D1 媒体 0、R2 对象 0，缺失对象、孤立对象、未关联 active 媒体和异常作品引用均为 0。
- 新增 `npm run db:recovery:preview`：只读导出指定 Preview D1，在系统临时目录恢复到隔离本地数据库，对比表计数、库存唯一索引和邮件出站触发器，最后自动删除临时导出。脚本固定目标为 `yx-art-studio-commerce-preview`，不接受 production 名称参数。
- 首次实测导出为 54,921 bytes；恢复后的 17 件作品、2 条纯测试订单、14 条事件、6 条假邮件出站、1 份资料、22 条内容记录和关键约束与远端一致。
- Cloudflare R2 账单面板本周期显示 `$0.00`、尚无可计费用量；A 类操作 6、B 类操作 16。应用 1 GiB / 200 对象保护仍不是账单硬停机上限。
- 当前 R2 为空，因此没有可保留的图片二进制用于完整恢复演练；正式上传真实作品前，应在含受控测试对象的独立环境补做“清单、下载、哈希、回传、再次核对”。
...
- 阶段 5E（Preview 运维一致性）：只读媒体一致性检查、Preview D1 临时导出/隔离恢复和首轮用量核对已完成；R2 当前为空，图片二进制完整恢复演练保留到独立受控对象阶段。

## 独立 Cloudflare Preview（截至 2026-08-26）
